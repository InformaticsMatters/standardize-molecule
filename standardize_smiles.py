...
Utility to standardize an input smiles into the standard representation used in the fragment network.
...
def get_smiles(isomericSmiles, std_molecule, osmiles):
    """Return Smiles of desired format given a standardised molecule
    """

    try:
        smiles = Chem.MolToSmiles(std_molecule, isomericSmiles, canonical=True)
    except Exception as e:
...
def process_osmiles(osmiles, isomericSmiles):
    """Takes original smiles and processes to return a smiles of the desired format (isomeric or nonisomeric)
    with the standardized molecule
    """
    mol = None
    try:
    ...
    if mol:
        ...
        if std_molecule:
            smiles = get_smiles(isomericSmiles, std_molecule, osmiles)
    else:
